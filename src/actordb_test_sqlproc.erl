...
-module(actordb_test_sqlproc).
-compile(export_all).
-include("actordb.hrl").
% 
% actordb_sqlproc callback module for isolated testing. 
% 
...
start(Name,Type1,Opt) ->
	...
	actordb_sqlproc:start([{actor,Name},{type,Type},{mod,?MODULE},
							  {state,#st{name = Name,type = Type}},{regname,{Name,Type}},test|Opt]).
...
start_steal(Name,Type1,Node,ShardName) ->
	...
	{ok,Pid} = actordb_sqlproc:start([{actor,Name},{type,Type},{mod,?MODULE},{state,#st{name = Name,type = Type,doreg = ShardName}},
									  {regname,{Name,Type}},{copyfrom,Node},{startreason,{steal,Node},test}]),
	{ok,Pid}.
...
cb_slave_pid(Name,Type,Opts) ->
	...
	case distreg:whereis(Actor) of
		undefined ->
			{ok,Pid} = actordb_sqlproc:start([{actor,Name},{type,Type},{mod,?MODULE},{slave,true},{regname,Actor},create,test|Opts]),
			{ok,Pid};
		Pid ->
			{ok,Pid}
	end.
