...
'sql'(Input, Index) ->
  p(Input, Index, 'sql', fun(I,D) -> (p_choose([fun 'select_query'/2, fun 'account_management_query'/2]))(I,D) end, fun(Node, _Idx) ->Node end).
...
'order_data'(Input, Index) ->
  p(Input, Index, 'order_data', fun(I,D) -> (p_seq([p_choose([p_seq([fun 'key'/2, p_string(<<".">>), fun 'key'/2]), fun 'key'/2, fun 'integer'/2]), p_optional(p_seq([fun 'space'/2, fun 'sort'/2]))]))(I,D) end, fun(Node, _Idx) ->
    case Node of
...
'table'(Input, Index) ->
  p(Input, Index, 'table', fun(I,D) -> (p_choose([fun 'table_alias'/2, fun 'table_value'/2]))(I,D) end, fun(Node, _Idx) ->Node end).
...
'param'(Input, Index) ->
  p(Input, Index, 'param', fun(I,D) -> (p_choose([fun 'param_value'/2, fun 'param_all'/2, fun 'param_all_alias'/2, fun 'param_key_alias'/2, fun 'param_key'/2]))(I,D) end, fun(Node, _Idx) ->Node end).
...
'set_comp'(Input, Index) ->
  p(Input, Index, 'set_comp', fun(I,D) -> (p_choose([fun 'not_in'/2, fun 'in'/2, fun 'exist'/2, fun 'all'/2, fun 'any'/2]))(I,D) end, fun(Node, _Idx) ->Node end).
...
'delete'(Input, Index) ->
  p(Input, Index, 'delete', fun(I,D) -> (p_regexp(<<"(?i)delete">>))(I,D) end, fun(_Node, _Idx) ->delete end).

-spec 'insert'(input(), index()) -> parse_result().
'insert'(Input, Index) ->
  p(Input, Index, 'insert', fun(I,D) -> (p_regexp(<<"(?i)insert">>))(I,D) end, fun(_Node, _Idx) ->insert end).
...
-spec 'not_in'(input(), index()) -> parse_result().
'not_in'(Input, Index) ->
  p(Input, Index, 'not_in', fun(I,D) -> (p_regexp(<<"(?i)not +in">>))(I,D) end, fun(_Node, _Idx) ->not_in end).
